...
public class ConceptRating {
	...
	public Vector<Concept> sortHashMapByValuesD(HashMap<String, Concept> passedMap){
		...
		concepts.addAll(passedMap.values());
	...
	public void writeJSON(PrintStream ps, boolean byLemma){
		...
		for(Concept c:list){
			...
			con.put("token", c.getToken().getText());
			con.put("lemma", c.getToken().getLemma());
			con.put("occurrences", new Integer(c.getCount()));
